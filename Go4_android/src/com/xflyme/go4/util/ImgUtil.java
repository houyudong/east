...
public class ImgUtil {
	...
	public static String imgToBase64(String imgPath, Bitmap bitmap) {
		...
		BitmapFactory.Options temOpts = new BitmapFactory.Options();
		temOpts.inJustDecodeBounds = true;
		BitmapFactory.decodeFile(imgPath, temOpts);
		ByteArrayOutputStream out = null;
		try {
			out = new ByteArrayOutputStream();
			// bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
			// int size = out.toByteArray().length / (1024 * 1024);
			...
			if (size >= 4) {
				out.reset();
				newOpts.inSampleSize = calculateOriginal(temOpts, 3000, 3000);
				bitmap = BitmapFactory.decodeFile(imgPath, newOpts);
				bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
			} else if (size >= 2) {
				out.reset();
				newOpts.inSampleSize = calculateOriginal(temOpts, 2000, 2000);
				bitmap = BitmapFactory.decodeFile(imgPath, newOpts);
				bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
			} else if (size >= 1) {
				out.reset();
				newOpts.inSampleSize = calculateOriginal(temOpts, 1500, 1500);
				bitmap = BitmapFactory.decodeFile(imgPath, newOpts);
				bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
			} else {
				out.reset();
				newOpts.inSampleSize = calculateOriginal(temOpts, 1000, 1000);
				bitmap = BitmapFactory.decodeFile(imgPath, newOpts);
				bitmap.compress(Bitmap.CompressFormat.JPEG, 100, out);
			}
	...
	private static int calculateOriginal(BitmapFactory.Options options,
		...
		while (reqWidth * reqHeight < height * width) {
	...
	public static void saveBitmapToFile(Bitmap bm, String path) {
		...
		try {
			...
			/*
			 * BitmapFactory.Options newOpts = new BitmapFactory.Options();
			 * newOpts.inPurgeable = true; newOpts.inJustDecodeBounds = false;
			 * newOpts.inPreferredConfig = Bitmap.Config.ARGB_8888;
			 * 
			 * if(size > 4){ newOpts.inSampleSize = 8;
			 * bm.compress(Bitmap.CompressFormat.JPEG, 25, out); }else if(size >
			 * 2){ newOpts.inSampleSize = 4;
			 * bm.compress(Bitmap.CompressFormat.JPEG, 50, out); }else if(size >
			 * 1){ newOpts.inSampleSize = 2;
			 * bm.compress(Bitmap.CompressFormat.JPEG, 75, out); }else{
			 * newOpts.inSampleSize = 1; bm.compress(Bitmap.CompressFormat.JPEG,
			 * 100, out); } out.flush(); out.close(); bm =
			 * BitmapFactory.decodeFile(path, newOpts); f.delete(); out = new
			 * FileOutputStream(f);
			 */
	 ...
	 * @return
	 */
	public static void cutBitmap(String imgPath, String newPath) {
		int size = 0;
		if (imgPath != null && imgPath.length() > 0) {
			File file = new File(imgPath);
			CLog.v("length", file.length() + "");
	...
	public static Bitmap readBitmap(String imgPath, int width, int height) {
		try {
			...
			newOpts.inJustDecodeBounds = false;
			newOpts.inPreferredConfig = Bitmap.Config.ARGB_8888;
			newOpts.inSampleSize = calculateOriginal(temOpts, width, height);
	...
	// public static String img_android =
	...
	// public static String time_img_jpg =
